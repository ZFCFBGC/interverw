...
// slice可以做到类数组转数组.改变数组的slice方法的作用域，在特定作用域中去调用slice方法，call（）方法的第二个参数表示传递给slice的参数即截取数组的起始位置。

Array.prototype.map.call(arrayLike, function(item) {
  return item.toUpperCase();
});
//Array.prototype.map.call解析：通过call调用后类数组也可以使用数组对应的方法
// ["NAME", "AGE", "SEX"]
```
...
foo("name", "age", "sex");
//打印出来的arguments是一个类数组{0: "john",1: "123",2: "M",length:3}
```
